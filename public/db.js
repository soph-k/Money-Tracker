...
const request = indexedDB.open("sophkbudget", 1);

// Set up your object store
// Think of an object store as a table inside your database
request.onupgradeneeded = ({ target }) => {
  let db = target.result;
  db.createObjectStore("sophkpending", { autoIncrement: true });
};
...
function saveRecord(record) {
  const transaction = db.transaction(["sophkpending"], "readwrite");
  const store = transaction.objectStore("sophkpending");
  store.add(record);
}
